...
describe('Test unpack()', () => {
  ...
  it('Should throw if called with null/undefined', () => {
    expect(() => unpack(null as unknown as JsonObject)).toThrow(
      InvalidJsonObjectException
    )
    expect(() => unpack(undefined as unknown as JsonObject)).toThrow(
      InvalidJsonObjectException
    )
    expect(() => unpack('hello' as unknown as JsonObject)).toThrow(
      InvalidJsonObjectException
    )
  })

  it('Should throw if no delimiter is specified', () => {
    expect(() => unpack({}, null as unknown as string)).toThrow(
      DelimiterNotSpecifiedException
    )
  ...
  it('Should throw if key names contain delimiter in their names', () => {
    ...
    expect(() => unpack(o, delimiter)).toThrow(
      new KeyNameContainsDelimiterException(key, delimiter)
    )
  })
